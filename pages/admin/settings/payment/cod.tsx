...
const CodForm = dynamic(() => import('@components/payments/cod-form'), {
  ssr: true
});
...
export default function Cod({ client }: SSRProps) {
  const {
    userInfo: { store: { etag } = {} }
  } = useGetClient(client);
  ...
  return (
    <>
      ...
      <CodForm initialValues={getPayment} />
    </>
  );
}
Cod.Layout = AppLayout;
